...
- [ ] 1.2 Include in the footprint every gap reconstruction must close to run the claim → verify: a dossier with a load-bearing dosing gap puts that gap in the footprint of every claim whose scenario doses — **open, and the verification is why**: a dossier records no per-claim scenario, so "the gaps *this claim's* run must close" is not derivable from what is stored. Adding every load-bearing gap to every claim is a different and weaker rule (it moves all footprints by a constant and can only make claims look less independent), and it would change every published footprint and every measured number in `docs/claim-selection.md`. Needs the scenario recorded per claim first

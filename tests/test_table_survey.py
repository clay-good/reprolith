...
def test_no_caption_states_a_model_result_worth_targeting() -> None:
    """Eleven candidates across ten papers, and not one of them names a quantity a model reports.
    ...
    The count was ten until the prose reader learned the other five classes' units; the one it
    gained is a number it could not previously see rather than a change of judgement about it.
    ...
    """
    assert sum(p["captions"]["candidates"] for p in _PAPERS.values()) == 11

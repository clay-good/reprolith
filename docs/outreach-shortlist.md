...
| **SED-ML and OMEX maintainers** | Reprolith's bundles are a private JSON record referencing an SBML file, not SED-ML or OMEX, so it has no *emitting* conformance story to offer yet. What it does have is intake experience: it reads SED-ML recipes, reads a document's plots as the claims a reproduction targets, and reads COMBINE archives — and it refuses several shapes rather than reinterpreting them, from a recipe whose sampling window it cannot carry to an archive that singles out no experiment. | Feed back the recipe shapes intake refuses and why. Offer archive conformance only once something emits an archive. |

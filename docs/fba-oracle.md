...
| `synthetic_lethal_genes` | Which *gene* pairs are viable to delete singly but lethal together — through the GPR rules? | the gene-level double deletion, the classic synthetic-lethality screen; a redundant isozyme or parallel enzyme is dispensable alone but lethal in pairs |

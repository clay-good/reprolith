...
rather than against a published result. That is a genuine validation of the machinery and it is not
a reproduction of anyone's published result: no row in the
[registry](../datasets/registry.html) comes from either path, because the registry is the six model
classes' blind runs over real models.

What each path *has* produced is a published certificate a reader can look at, which for a while it
had not:
[`datasets/population/worked_example/`](../datasets/population/worked_example/) and
[`datasets/estimation/worked_example/`](../datasets/estimation/worked_example/) carry the whole walk
— model, ensemble or fit, verdict — with nothing hand-written between them, each beside the
`reference.json` it was judged against and each regenerated by `scripts/render_worked_examples.py`.
Until then both halves were demonstrated only inside a test, which is the state this repository has
had to correct in itself more than once: a capability whose only evidence is a test is a capability
nobody outside can check.

The two renders are honest about what they are in their own text. The population one is
`partially-reproduced` on a claim that reproduces cleanly, because the envelope rests on a
variability model and a sampling Reprolith chose; the estimation one says on its own claim line that
recovering a parameter is not the same as reproducing the results that parameter produces.

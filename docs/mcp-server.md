...
| `verification_queue` | — | The load-bearing values every standing certificate rests on, most consequential first: the question, Reprolith's estimate, its basis, the alternatives, and the certificates that would have to be re-issued if an expert corrected it. Split into what an expert can decide and what only this engine's development can close, and the split moves as the engine changes — a spatial claim can carry the wall its source states, so that question left the engine's half, and the stochastic class's first-passage entry added one to it. The counts are in the report rather than written down here, where they have gone stale twice. Derived from the ledger on every call, so a superseded certificate is not a live dependency. An item an expert has answered (`datasets/verification_decisions.json`) moves to `decided` with the expert, date, rationale and source, and keeps saying that its certificates are still qualified — a decision is an answer, not a re-certification. A decision whose question has since been reworded is reported as `stale_decisions` and its item is pending again |
| `recertification_due` | — | Which standing certificates owe a re-run, and why. Two causes, named apart: an expert **corrected** a value they rest on, and the **judging code has moved** since they were issued, so their numbers are not the ones this checkout produces. A confirmation owes nothing — the value is still one Reprolith chose and the numbers do not move. Dependents of a rejected estimate are `blocked` rather than due, since a rejection supplies no replacement value, and a certificate whose class the surface cannot name is reported as unchecked rather than passed. It re-runs nothing: re-issuing is `reverify_dependents` with a real re-certification behind it |
| `loop_status` | — | Whether there is gated, publishable work, and if not exactly why not: `stop_reason` names the cause — everything blocked on one input, everything parked, entries with no accession, or a genuinely empty backlog, which are four different situations. Carries the parks with their diagnoses and the escalations split into what an expert can decide and what only this engine can. `standing` counts what is published and says plainly that it is not a record of any one run |
| `verification_issue` | `item_id` (required) | One queue item as the GitHub issue to open: title, the three labels the collaboration spec asks for (model class, impact rank, pending-verification status), and a body with the question, the estimate and its basis, the alternatives, and the papers that rest on it. Refuses an item that is a limit of this engine rather than a question a paper could answer. It files nothing — a person opens it. `existing_decisions` says whether somebody already answered |
...
| `select_claims` | `accession`, `budget` | Which of a paper's claims to reproduce within a budget: the set maximizing independent evidential value, the plain one-at-a-time ranking beside it for comparison, and what the dossier records nothing for. A plan, not a verdict — no model is run |
...
| `lint_stochastic` | `sbml`, `species`, `reported_mean`, `duration`, `trajectories`, `seed` | A deterministic verdict on an SBML reaction network's mean species count via a pinned Gillespie SSA, with what the ensemble cost on its protocol line — the standard error, and the trajectory count that would settle the claim (needs the engine extra) |
...
| `claim_work` | `requester` (required), `model_class`, `lease_seconds` (default 3600), `include_parked` (default false) | Claims the next best claimable entry and leases it to the requester, so agents sharing one server don't collide. Readier (lower-difficulty) work is offered first; ground truth is deliberately not a ranking key, and an expired lease returns the entry to the pool. Returns the leased entry or that there is no eligible work. An entry carrying no accession cannot be finished or released — both address an entry by accession — so it is stepped over rather than offered, and `skipped_without_accession` reports how many were passed over, whether or not one was claimed. A refusal also carries `blocked_on` — what the blocked entries are waiting on, counted — so "no eligible work" in front of a 31-entry backlog names the capability that would release it instead of stopping there. An entry claimed three times in a row with no lifecycle transition between the claims is **parked**: it stops being offered, because an abandoned claim ends by lease expiry and records nothing, so an entry that defeats everyone who takes it was handed out again immediately — and readier work ranks first, which put an easy one of those at the head of the queue permanently. The refusal names each park with its diagnosis; `include_parked` takes one anyway, deliberately, and the reply says so. The park clears itself on any transition, so nothing has to un-park it. |
| `release_work` | `accession`, `requester`, `reason` | Releases a claimed entry back to the queue. Only the lease holder may release it. `reason` is recorded on the attempt, and is what lets a parked entry report whether everyone who tried hit the same wall — the difference between three problems and one, as `blocked_on` is for blocked entries. Every distinct reason is listed with its count, so the one claimant who found something different is not hidden behind the majority. An attempt that ends by lease expiry records nothing, and the diagnosis says so rather than reading as a park whose reasons were unremarkable. |
...
| `reprolith claims-propose` | It reads the tables an author's *paper* prints, and with `--prose` its running text — files the server does not hold, for a paper that has no entry here yet — and its output is meant to be edited by hand before it is used, which is a file on disk. |
...
| `reprolith params-propose` | It reads the tables an author's own *paper* prints, and with `--prose` its running text, like `claims-propose`, and writes them into the shape `params-check` reads. Same two reasons: a file the server does not hold, and output meant to be edited by hand before it is used. |

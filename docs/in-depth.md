...
*The full account: what is built, what each certificate rests on, every command, and the
numbers behind each claim. The short version is the [README](../README.md).*
...
a sentence it is the only reader there is. So it ships as `--prose` on both commands that read a
paper — `claims-propose` for its results and `params-propose` for its inputs — merged into the
tables' own candidates file. What it does not do is de-duplicate: a value printed in a table and
restated in a sentence is two citations, and choosing between them is the curator's judgment.
...
[`docs/figure-values.md`](figure-values.md).
[`docs/findings-note.md`](findings-note.md) and [`openspec/`](../openspec/) say so in detail. Where a paper ships a **SED-ML** document, the half of that
...
[`docs/sedml-fast-path.md`](sedml-fast-path.md).
...
- **An answer to "can it check the thing my paper reports?"** Seventeen kinds of result can be
  ...
  [`docs/claim-types.md`](claim-types.md) lists every one with what it is compared against and
...
- **Standard, runnable artifacts.** The model ships as SBML and the engine is pinned by
  ...
  ([`docs/sedml-fast-path.md`](sedml-fast-path.md)). The metformin worked example ships its
...
- **A plan for a budget you can actually afford.** A paper's thirty-three published numbers are
  ...
  ([`docs/claim-selection.md`](claim-selection.md)).
...
because that line used to say less the worse its news was. It is reachable from the terminal and over MCP, not only from the
  published page ([`docs/self-validation.md`](self-validation.md)).
...
`pip install -e .`); see [docs/mcp-server.md](mcp-server.md) to register it in a client and
...
[`docs/author-check.md`](author-check.md) is the guide for the author running it: the two
...
> [`docs/discipline-loop.md`](discipline-loop.md).
...
[docs/self-validation.md](self-validation.md) both spell out why.
...
[docs/fba-oracle.md](fba-oracle.md). The same shared pathway carries it end to end: a
...
[docs/kinetic-class.md](kinetic-class.md).
...
agreement machinery. See [docs/logical-class.md](logical-class.md).
...
[docs/population-and-estimation.md](population-and-estimation.md).
...
machinery — [docs/self-validation.md](self-validation.md) is the one-look evidence summary, and
[docs/claim-types.md](claim-types.md) is the one-look answer to "can it check the thing my

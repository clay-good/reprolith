...
## Two capabilities whose only evidence was a test

`simulate_population` and `refit_parameters` are the producing halves of two reproduction levels
whose oracles shipped first. Both are built, both are validated against closed forms, and until now
neither had produced a single artifact a reader could look at:
`docs/population-and-estimation.md` had to say, in as many words, that no row in the registry comes
from either path. The demonstration of each lived inside a test.

That is a state this repository has had to correct in itself repeatedly — the FROG fingerprint, the
basin of attraction, the noise statistics — and the correction is always the same: publish the
artifact. `datasets/population/worked_example/` and `datasets/estimation/worked_example/` carry the
whole walk with nothing hand-written between the model and the verdict, each beside the
`reference.json` it was judged against, each regenerated by the script that writes the other three
worked examples, and each gated the way they are: the render must be the rendering of the committed
certificate, byte for byte, so a hand-edited verdict in a published text is a failure rather than a
publication.

Neither is a paper. The population reference is the closed-form percentiles of a log-normal
one-compartment model and the estimation data is that model's own trajectory — mathematics standing
in for a figure and for a dataset, which each certificate says on its own claim line. What they
demonstrate is that the paths run end to end and land where the mathematics says, which is exactly
what the six milestones demonstrate for the classes.

Reading them as their reader corrected two of my own assumptions rather than the code's. A
population verdict is filed at **simulation** level, not at a population level — the two levels this
engine distinguishes are running the described model and re-fitting it from data, and a population
is the first of those run many times; what makes it a different kind of result is the comparison and
the assumption, both already on the certificate. And the estimation certificate's "what was missing"
line is derived from the claim's level when the certificate is *rendered*, so it is in the render

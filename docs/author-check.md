...
| --- | --- |
| **Does your model name its own quantities?** | A `<species>` with no `id`, or a `<rateRule>` with no `variable`, is not a formatting slip — the model no longer says which quantity it means. libSBML reports these as *errors*, not fatal ones, and still hands the document back as a model, so a reader that refuses only fatal files accepts it. The two engines then disagree about what you shipped: COPASI refuses the import with a raw libSBML code and a line number, and libRoadRunner runs the file to completion having invented a binding for each anonymous element — measured on one such model, an unnamed species ran from 5 to 10 under an unnamed rate rule, beside the correct decay of the named one, with nothing printed. This is the first thing in the fix list, above everything below. Reprolith emitted such a file itself until its writers began refusing the names that cause it: libSBML's setters reject an invalid identifier by returning a code rather than raising. |
| **Does your experiment name its own parts?** | Everything in a SED-ML document refers to everything else by id, so a `<task>` or `<dataGenerator>` without one cannot be referred to and is not read at all. Nothing reported it: changing that single attribute took "runs a reproducer can adopt verbatim" from 1 to 0, with no line saying which run went missing. The count still drops; now something says why. |
...
| **Can your own recorded data be read?** | A `dataDescription` in a format this does not parse, naming a source your archive does not contain, or whose `dataSource` selects more than one column or a column its file does not name, is not read — and the only symptom is a claim with no reference values, which is exactly what the report says about a paper that published none. Each reason is now named against the description or the source it is about. An `indexSet` source is not one of them: naming an index column is a normal part of a conformant document. |
| **Does it say what you published?** | A document whose outputs are all `report`s states no published result: it can be run, but there is nothing to check it against. A `plot` is your own statement that a curve is a shown result. If your paper prints its results as *numbers* rather than plotting them, no document can say so — SED-ML has no way to state "the peak of this curve is the published value" — so the route is `--claims`, below, and the fix list says so rather than asking you to plot a curve you never showed. Once you pass one, the item stops asking for it and says the part that is still true: your results were checked here, from a file the archive does not carry, so somebody who downloads the archive alone still has nothing. |
